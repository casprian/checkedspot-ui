  ...
  <v-expand-transition>
    ...
    >
      <div
        ...
        class="text-h5 text-center d-flex align-center justify-center"
      >
        <h5>Enquiry request is successful!</h5>
      </div>
    </v-card>
  </v-expand-transition>
  <v-container fluid class="pa-0 pt-10 px-4 bg-background" style="height: 100%;">
    <v-row
      v-if="(count < 1) && !errorOccured"
      no-gutters
    ...
    <v-row v-else-if="errorOccured" no-gutters class="px-sm-10 d-flex justify-center align-center" style="height: 100%;">
        <v-col cols="12" class="pr-0 pr-md-3 py-10">
    ...
    <v-row v-else no-gutters class="px-sm-10">
      <v-col cols="12" md="8" class="pr-0 pr-md-3">
        <v-row no-gutters class="mb-10">
          <v-col cols="12" sm="9">
            <div
              class="d-flex justify-space-between align-center justify-sm-start"
            >
              ...
              <div v-if="property?.data">
                <div
                  v-if="property?.data.propertyStatus"
                  class="px-4 py-1 text-body-1 rounded-pill ml-6 bg-pink-accent-3"
                >
                  {{
            ...
            <div class="py-2">
              ...
              ></v-icon>
              <span
                class="text-body-1 text-md-h6 font-weight-regular text-grey-darken-2"
                >{{
                  property?.data?.city ? `${property?.data?.city}` : "Not Found"
                }}</span
              >
            </div>
          </v-col>
          <v-col
            cols="12"
            sm="3"
            class="d-flex flex-column align-start align-sm-end"
          >
            <div
              class="text-h6 text-md-h5 font-weight-medium text-pink-accent-3"
            >
              {{
                property?.data?.cost
                  ? `₹ ${
                      property?.data?.cost < 10000000
                        ? `${(property?.data?.cost / 100000.0).toFixed(2)} Lac`
                        : `${(property?.data?.cost / 10000000.0).toFixed(2)} Cr`
                    }`
                  : ""
              }}
            </div>
        ...
        <v-row no-gutters class="mb-8" style="width: 100%; height: auto">
          <property-image
            ...
            :image="property?.data?.image?.fileUrl"
          />
        </v-row>
        ...
        <v-row class="ma-0 mb-8" style="width: 100%; height: auto">
          <v-col cols="12" class="pa-0">
            <v-card width="100%" class="rounded-0 px-2 pb-4 pt-2" elevation="2">
              ...
              <v-row
                no-gutters
                class="px-4 pb-5 d-flex justify-space-around align-center"
              >
                <v-col
                  cols="auto"
                  class="my-3"
        ...
        <!-- Location -->
        <v-row
          v-if="property?.data?.longitude && property?.data?.latitude"
          no-gutters
          class="mb-8"
        >
        ...
        <v-row no-gutters class="mb-8">
          <property-video
            :propertyId="property?.data?.propertyId"
            :videosExist="property?.data?.videosExist"
          />
        </v-row>
      </v-col>
        ...
        <v-row no-gutters>
          ...
          <v-col v-if="isSold === true" cols="12">
            <v-card class="rounded-0" elevation="2">
              <v-card-item>
                <div class="text-h3 font-weight-regular pa-10 bg-yellow">
                  Property is Sold!
                </div>
              </v-card-item>
            </v-card>
...
const PropertyDetails = defineAsyncComponent(
  () =>
    import("@/components/property-details-view-components/PropertyDetails.vue")
);
const PropertyVideo = defineAsyncComponent(
  () =>
    import("@/components/property-details-view-components/PropertyVideo.vue")
);
const PropertyImage = defineAsyncComponent(
  () =>
    import("@/components/property-details-view-components/PropertyImage.vue")
);
const PropertyDocument = defineAsyncComponent(
  () =>
    import("@/components/property-details-view-components/PropertyDocument.vue")
);
const PropertyAgents = defineAsyncComponent(
  () =>
    import("@/components/property-details-view-components/PropertyAgents.vue")
);
const PropertyLocationMap = defineAsyncComponent(
  () =>
    import(
      "@/components/property-details-view-components/PropertyLocationMap.vue"
    )
);
const PropertyDescription = defineAsyncComponent(
  () =>
    import(
      "@/components/property-details-view-components/PropertyDescription.vue"
    )
);
...
//form validation
let { meta, values, errors, handleSubmit, handleReset, defineComponentBinds } =
  useForm({
    validationSchema: {
      name(value: any) {
