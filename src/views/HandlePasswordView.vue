<template>
  <v-alert
    v-model="otpDelivered"
    location="top center"
    elevation="2"
    type="success"
    closable
    title="OTP Sent"
    :text="'OTP has been sent to ' + email.value.value"
  ></v-alert>
  <v-container fluid class="pt-16 pb-16 d-flex align-center wallpaper">
    <v-row v-if="!verifiedUser" no-gutters class="d-flex justify-center">
      <v-col cols="12" sm="5">
        <v-card
          class="pa-5 pb-sm-10 pt-sm-8 bg-grey-lighten-5 rounded-xl"
          elevation="8"
        >
          <div class="mx-md-10 mb-7 text-h5 text-md-h4 text-grey-darken-3">
            User Verification
          </div>
          <v-text-field
            class="my-2 mx-md-10"
            :disabled="!sendBtn ? true : false"
          ...
          <div class="d-flex flex-column align-center justify-center">
            <div v-if="!sendBtn" class="text-caption mb-5 text-center">
              if not received OTP on click of SEND OTP Button. Click on RESEND
              OTP Button which will appear after 30 seconds.
            </div>
            <div>
              <v-btn
                v-if="sendBtn"
            ...
            <div>
              <v-btn color="red" width="300" variant="tonal" @click="deleteOTP"
                >Close</v-btn
              >
            </div>
          </div>
    ...
    <v-dialog v-model="wrongOTPEntered" width="auto">
      <v-card
        max-width="400"
        text="You have entered wrong OTP. Please Enter same OTP sent in your registered email."
        title="Invalid OTP"
      >
        <template v-slot:actions>
          <v-btn
            class="ms-auto"
            text="Ok"
            @click="wrongOTPEntered = false"
          ></v-btn>
        </template>
      </v-card>
    </v-dialog>
    <v-dialog v-model="otpExpired" width="auto">
      <v-card
        max-width="400"
        text="OTP is valid for 5 minutes Only. Please resend OTP!"
        title="Invalid OTP"
      >
        <template v-slot:actions>
          <v-btn class="ms-auto" text="Ok" @click="otpExpired = false"></v-btn>
        </template>
      </v-card>
    </v-dialog>

    <forget-password
      v-if="route?.query?.q === 'forget' && verifiedUser"
      :email="email.value.value"
    />
    <reset-password
      v-if="route?.query?.q === 'reset' && email"
      :email="email"
    />
  </v-container>
</template>
...
const wrongOTPEntered = ref(false);
...
async function verifyEmail() {
  wrongOTPEntered.value = false;
  loader2.value = true;
  if (validOtp.value) {
    const res = await api?.email?.verifyEmail({
      email: email?.value?.value,
      OTP: otp?.value?.value,
    });
    if (res?.status === 200) {
