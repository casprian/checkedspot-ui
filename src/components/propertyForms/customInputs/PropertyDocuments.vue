<template>
    <div>sdlkfjalksjfla</div>
    <v-col cols="auto" class="ma-2" v-for="document in props.documents" :key="document.type">
            ...
            <v-card class="pa-5 pt-2" color="grey-lighten-3" :elevation="isHovering ? 10 : 2" v-bind="props">
                ...
                <v-row class="d-flex flex-column justify-center align-center">
                    <v-col cols="auto" class="pb-0">
                        <label :for="`plotform${document.type}`" class="docfile">
                            <v-icon icon="mdi-file" size="100" color="grey-darken-1"></v-icon>
                            <input type="file" accept=".pdf" :id="`plotform${document.type}`" style="width: 0;" @change="uploadDoc">
                        </label>
                    </v-col>
    
                    <v-col cols="12" class="pt-0" style="min-width: 180px;">
