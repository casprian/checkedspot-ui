      ...
      >
        <h5>
          ...
          <span class="text-body-1"
            >Please refresh page if changes are not visible.</span
          >
        </h5>
      </div>
    </v-card>
        ...
        <v-col cols="12" v-for="image in images" :key="image.id" class="my-3">
          <v-row no-gutters class="pa-0">
            <v-col cols="12" md="6" class="pa-0">
              <img :src="image?.fileUrl" class="imagestyle text-right" />
            </v-col>
            <v-col
              cols="12"
              md="6"
              class="imageDetailsSec"
              style="height: auto"
            >
              <div class="title">
                ...
                <p class="text-body-2">
                  {{
                    image.title
                      ? image.title
                      : "No title assigned to the image."
                  }}
                </p>
              </div>
    ...
    <v-row no-gutters class="mt-8">
      <v-col cols="12" class="d-flex justify-center">
        <v-btn
          ...
          color="primary"
          width="200"
          @click="addImageDialog = !addImageDialog"
          >Add Image</v-btn
        >
        ...
        <v-dialog v-model="addImageDialog" width="auto">
          <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
              <v-col cols="12" class="text-h6 pb-5"> Add new image </v-col>
              <v-col cols="12">
                <v-file-input
                  v-model="addImages"
                  label="File input"
...
async function fetchPropertyImages() {
  images.value = [];
  const res = await api?.property?.getPropertyImage({
