<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Air Conditioning : &nbsp; <span :class="airConditioning === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.airConditioning }}</span>
        </span>
        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Air Conditioning
                </v-col>
                <v-col cols="12">
                    <v-select v-model="airConditioning" :items="status" label="air conditioning" variant="outlined" clearable
                        hint="choose for the availability of air conditioning in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'airConditioning']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const airConditioning = ref(props.airConditioning);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "airConditioning": airConditioning.value }
    });
