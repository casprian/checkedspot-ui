<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Parking Lot : &nbsp; <span :class="parkingLot === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.parkingLot }}</span>
        </span>
        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Parking Lot
                </v-col>
                <v-col cols="12">
                    <v-select v-model="parkingLot" :items="status" label="parking lot" variant="outlined" clearable
                        hint="choose for the availability of parking lot in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'parkingLot']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const parkingLot = ref(props.parkingLot);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "parkingLot": parkingLot.value }
    });
