<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Gym : &nbsp; <span :class="gym === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.gym }}</span>
        </span>
    ...
    <v-dialog v-model="dialog" width="auto">
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                ...
                <v-col cols="12">
                    <v-select v-model="gym" :items="status" label="gym" variant="outlined" clearable
                        hint="choose for the availability of gym in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'gym']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const gym = ref(props.gym);
