        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property TV Cable
                </v-col>
                <v-col cols="12">
                    <v-select v-model="tvCable" :items="status" label="tv Cable" variant="outlined" clearable
                        hint="choose for the availability of tvCable in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'tvCable']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const tvCable = ref(props.tvCable);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "tvCable": tvCable.value }
    });
