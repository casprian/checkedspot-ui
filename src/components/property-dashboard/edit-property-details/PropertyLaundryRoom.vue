<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Laundry Room : &nbsp; <span :class="laundryRoom === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.laundryRoom }}</span>
        </span>
        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Laundry Room
                </v-col>
                <v-col cols="12">
                    <v-select v-model="laundryRoom" :items="status" label="laundry room" variant="outlined" clearable
                        hint="choose for the availability of laundry room in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'laundryRoom']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const laundryRoom = ref(props.laundryRoom);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "laundryRoom": laundryRoom.value }
    });
