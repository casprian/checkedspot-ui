<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;"> 
            Property Visibility : &nbsp; <span class="text-blue-grey-darken-1">{{ props.visibility }}</span>
        </span>
            ...
            <v-row no-gutters class="pa-10 pt-7">
                ...
                <v-col cols="12">
                    <v-select 
                        ...
                        variant="outlined" 
                        clearable
                        hint="Choose status of the property."
                    ></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'visibility']);
...
const status = ref(["public", "private"]);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "visibility": visibility.value }
    });
