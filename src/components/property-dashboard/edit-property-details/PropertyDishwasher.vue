<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Dishwasher : &nbsp; <span :class="dishWasher === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.dishWasher }}</span>
        </span>
        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Dishwasher
                </v-col>
                <v-col cols="12">
                    <v-select v-model="dishWasher" :items="status" label="dishwasher" variant="outlined" clearable
                        hint="choose for the availability of dishwasher in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'dishWasher']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const dishWasher = ref(props.dishWasher);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "dishWasher": dishWasher.value }
    });
