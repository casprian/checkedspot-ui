<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Garden : &nbsp; <span :class="garden === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.garden }}</span>
        </span>
        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Garden
                </v-col>
                <v-col cols="12">
                    <v-select v-model="garden" :items="status" label="garden" variant="outlined" clearable
                        hint="choose for the availability of garden in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'garden']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const garden = ref(props.garden);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "garden": garden.value }
    });
