        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Swimming Pool
                </v-col>
                <v-col cols="12">
                    <v-select v-model="swimmingPool" :items="status" label="swimming pool" variant="outlined" clearable
                        hint="choose for the availability of swimming pool in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'swimmingPool']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const swimmingPool = ref(props.swimmingPool);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "swimmingPool": swimmingPool.value }
    });
