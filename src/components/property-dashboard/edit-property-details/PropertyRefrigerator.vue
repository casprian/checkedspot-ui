        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Refrigerator
                </v-col>
                <v-col cols="12">
                    <v-select v-model="refrigerator" :items="status" label="refrigerator" variant="outlined" clearable
                        hint="choose for the availability of refrigerator in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'refrigerator']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const refrigerator = ref(props.refrigerator);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "refrigerator": refrigerator.value }
    });
