<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Elevator : &nbsp; <span :class="elevator === 'available'  ? 'text-blue-grey-darken-1' : 'text-red-darken-1'">{{ props.elevator }}</span>
        </span>
        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Elevator
                </v-col>
                <v-col cols="12">
                    <v-select v-model="elevator" :items="status" label="elevator" variant="outlined" clearable
                        hint="choose for the availability of elevator in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'elevator']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const elevator = ref(props.elevator);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "elevator": elevator.value }
    });
