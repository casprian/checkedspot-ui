        ...
        <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Property Lobby
                </v-col>
                <v-col cols="12">
                    <v-select v-model="lobby" :items="status" label="Lobby" variant="outlined" clearable
                        hint="choose for the availability of lobby in property."></v-select>
                </v-col>
...
const props = defineProps(['propertyId', 'lobby']);
const emit = defineEmits(['success', 'failure']);
const dialog = ref(false);
const loader = ref(false);

const lobby = ref(props.lobby);
...
async function update() {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "lobby": lobby.value }
    });
