<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Number of Bathroom : &nbsp; <span class="text-blue-grey-darken-1">{{ props.noOfBathroom }}</span>
        </span>
        ...
        <v-card width="50vw" color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
                <v-col cols="12" class="text-h6 pb-5">
                    Update Number of Bathroom
                </v-col>
                <v-col cols="12">
                    <v-text-field label="Number of Bathroom (required)" v-model="noOfBathroom.value.value"
...
const props = defineProps(['propertyId', 'noOfBathroom']);
const emit = defineEmits(['success', 'failure'])
const dialog = ref(false);
const loader = ref(false);

const bathrooms = ref(props.noOfBathroom);
...
noOfBathroom.value.value = bathrooms.value;
...
async function onSuccess(values: any) {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "noOfBathroom": parseInt(values.noOfBathroom) }
    });
