<template>
    <div class="my-4 d-flex justify-space-between align-center">
        <span class="wraplink" style="width:85%;">
            Number of Bedrooms : &nbsp; <span class="text-blue-grey-darken-1">{{ props.noOfBedroom }}</span>
        </span>
...
const props = defineProps(['propertyId', 'noOfBedroom']);
const emit = defineEmits(['success', 'failure'])
const dialog = ref(false);
const loader = ref(false);

const bedrooms = ref(props.noOfBedroom);
...
noOfBedroom.value.value = bedrooms.value;
...
async function onSuccess(values: any) {
    ...
    const res = await api?.property?.updateDetails({
        ...
        "updatingFields": { "noOfBedroom": parseInt(values.noOfBedroom) }
    });
