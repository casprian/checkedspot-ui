    ...
    <v-row no-gutters>
      ...
      <v-row no-gutters>
        <v-col cols="12" v-for="video in videos" :key="video.id" class="my-3">
          <v-row no-gutters>
            <v-col cols="12" md="6">
              <video muted controls class="videostyle">
              ...
              <div class="title">
                ...
                <p class="text-body-2">
                  {{
                    video.title
                      ? video.title
                      : "No title assigned to the Video."
                  }}
                </p>
              </div>
    ...
    <v-row no-gutters class="mt-8">
      <v-col cols="12" class="d-flex justify-center">
        <v-btn
          ...
          color="primary"
          width="200"
          @click="addVideoDialog = !addVideoDialog"
          >Add Video</v-btn
        >
        ...
        <v-dialog v-model="addVideoDialog" width="auto">
          <v-card color="grey-lighten-5">
            <v-row no-gutters class="pa-10 pt-7">
              <v-col cols="12" class="text-h6 pb-5"> Add new Video </v-col>
              <v-col cols="12">
                <v-file-input
                  v-model="addVideos"
                  label="File input"
...
async function fetchPropertyVideos() {
  videos.value = [];
  const res = await api?.property?.getPropertyVideo({
