    ...
    >
      <v-form>
        <v-row no-gutters>
          <v-col
            cols="12"
            class="px-10 py-5 mb-5 elevation-2 d-flex justify-space-between align-center bg-white"
          >
...
const profilePicture = ref(
  "https://checkedspot.blob.core.windows.net/assets/loader.gif"
);
...
watch(selectedState, (newStateItem) => {
  ...
  const selectedState = location.states.find(
    (state: any) => state?.name === newStateItem
  );
...
async function getUser() {
  ...
  if (res?.status === 200) {
    ...
    role.value =
      res?.data?.data?.roles[0] === "user"
        ? "individual"
        : res?.data?.data?.roles[0];
  }
...
onMounted(async () => {
  ...
  cityitems.value = location.states.find(
    (state: any) => state.name === userdata.value.state
  )?.cities;
});
</script>
