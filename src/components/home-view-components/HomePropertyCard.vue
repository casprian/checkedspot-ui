<template>
  <v-card
    class="mx-2 mb-4 elevation-2"
    style="max-width: 220px"
    position="relative"
  >
    <v-hover v-if="!isMobileDevice" v-slot="{ isHovering, props }">
      <v-img
        @click.stop.prevent="openPropertyDetail"
        v-bind="props"
        :src="
      ...
      >
        ...
        <v-toolbar
          v-if="property?.title && property?.title !== 'unavailable'"
          class="propertyTitle"
          theme="dark"
          height="30"
        ...
        <div class="verifiedTagContainer">
          <v-card-title
            title="verified by Plotwala"
            v-if="property?.isVerifiedByCheckedSpot"
      ...
      <div class="showDetailsBtnContainer">
        <div class="d-flex flex-column flex-md-row justify-center align-center">
          <v-btn
            density="compact"
            variant="elevated"
            class="text-none text-caption my-1"
            color="red-darken-3"
            append-icon="mdi-cursor-default-click"
            >For details. Click here!</v-btn
          >
          <v-btn
            density="compact"
            class="text-none text-caption ml-2 my-1"
            color="primary"
            @click.prevent.stop="
              () => {
    ...
    <v-row no-gutters class="mt-1">
      <v-col cols="6" class="pa-0 pl-2 mb-1" style="height: 20px">
        ...
        >
          <v-icon
            ...
            color="grey-darken-2"
          ></v-icon>
          <span class="text-body-2 text-grey-darken-2" title="google map link"
            >Location</span
          >
        </a>
      </v-col>
        ...
        <v-row no-gutters v-bind="props" class="cardBottom py-2">
          <v-row no-gutters v-if="!isHovering" class="px-4">
            <v-col cols="auto" class="px-0 d-flex justify-center align-center">
              ...
              ></v-avatar>
              <div
                class="text-body-2 text-uppercase text-center text-grey-darken-2"
              >
                <p
          ...
          <v-expand-transition>
            <div
              v-if="isHovering"
              class="d-flex justify-center align-center v-card--reveal"
      ...
      <v-row v-else no-gutters v-bind="props" class="cardBottom">
        <v-row
          no-gutters
          class="pa-0 mx-6 d-flex justify-space-between align-center"
        >
          <v-col cols="auto" class="px-0 d-flex justify-center align-center">
            <v-avatar
              class="pa-0 mr-2"
            ...
            ></v-avatar>
            <div
              class="text-body-2 ml-0 text-uppercase text-center text-grey-darken-2"
            >
              <p
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue-darken-1" variant="text" @click.stop.prevent="handleReset">
            Reset
          </v-btn>
          <v-btn
            color="blue-darken-1"
            variant="text"
            @click.stop.prevent="enquiryForm = false"
          >
            Close
          </v-btn>
          <v-btn
            color="blue-darken-1"
            variant="elevated"
            @click.stop.prevent="handleEnquiry"
